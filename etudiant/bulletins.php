...
$page_title = 'Mes Bulletins';
$db = Database::getInstance()->getConnection();

// Récupérer l'étudiant avec informations utilisateur
...
// Créer le bulletin pour le semestre
$bulletins = [];
if (!empty($notes)) {
    ...
    foreach ($classement as $index => $row) {
        if ($row['etudiant_id'] == $etudiant['id']) {
            $rang = $index + 1;
            break;
        }
    }
    ...
    $bulletins[1] = [
        'periode' => ['libelle' => 'Semestre ' . $semestre_filter, 'date_debut' => date('Y-01-01'), 'date_fin' => date('Y-12-31')],
        'matieres' => $matieres,
...
<div class="row mb-4">
    <div class="col-12">
        <h2><i class="fas fa-file-alt"></i> Mes Bulletins</h2>
